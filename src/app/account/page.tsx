...
export default function AccountPage() {
      ...
      <div className="min-h-screen bg-ivory flex items-center justify-center">
        <div className="w-full max-w-md px-4">
          <div className="bg-white border border-border rounded-lg p-6 sm:p-8">
            <h1 className="font-display text-2xl sm:text-3xl text-center mb-2">Welcome Back</h1>
            <p className="text-xs sm:text-sm text-warm-gray text-center mb-6 sm:mb-8">Sign in to your Lumière account</p>
            <form onSubmit={(e) => { e.preventDefault(); login(loginForm.email, loginForm.password); }} className="space-y-3 sm:space-y-4">
  ...
  return (
    <div className="min-h-screen bg-ivory">
      <div className="max-w-6xl mx-auto px-4 sm:px-5 md:px-8 lg:px-12 py-8 sm:py-10 md:py-12">
        <div className="flex items-center justify-between mb-6 sm:mb-8 gap-4">
          <div>
            <h1 className="font-display text-2xl sm:text-3xl font-light">My Account</h1>
            <p className="text-xs sm:text-sm text-warm-gray mt-1">Welcome back, {customer?.name}</p>
          </div>
          <button onClick={logout} className="flex items-center gap-1.5 sm:gap-2 text-xs sm:text-sm text-warm-gray hover:text-rose transition-colors shrink-0">
            <LogOut size={14} className="sm:w-4 sm:h-4" /> Sign out
          </button>
        </div>
        ...
        <div className="grid lg:grid-cols-4 gap-6 sm:gap-8">
          ...
          <div className="lg:col-span-1">
            <nav className="bg-white border border-border rounded-lg overflow-hidden flex lg:flex-col overflow-x-auto">
              {tabs.map((tab) => (
                <button key={tab.id} onClick={() => setActiveTab(tab.id)}
                  className={`flex items-center gap-2 sm:gap-3 w-full min-w-fit px-4 sm:px-5 py-3 sm:py-4 text-xs sm:text-sm transition-colors border-b lg:border-b border-border last:border-0 ${activeTab === tab.id ? 'bg-cream text-gold font-medium' : 'text-warm-gray hover:bg-ivory'}`}>
                  <tab.icon size={16} className="sm:w-[18px] sm:h-[18px] shrink-0" />
                  <span className="whitespace-nowrap">{tab.label}</span>
                  <ChevronRight size={14} className="ml-auto hidden lg:block" />
                </button>
              ))}
              <Link href="/wishlist" className="flex items-center gap-2 sm:gap-3 w-full min-w-fit px-4 sm:px-5 py-3 sm:py-4 text-xs sm:text-sm text-warm-gray hover:bg-ivory transition-colors">
                <Heart size={16} className="sm:w-[18px] sm:h-[18px] shrink-0" /> <span className="whitespace-nowrap">Wishlist</span> <ChevronRight size={14} className="ml-auto hidden lg:block" />
              </Link>
            </nav>
          </div>
          ...
          <div className="lg:col-span-3">
            {activeTab === 'orders' && (
              <div>
                <h2 className="font-display text-lg sm:text-xl mb-4 sm:mb-6">Order History</h2>
                {sampleOrders.length === 0 ? (
                  <div className="text-center py-8 sm:py-12 bg-white border border-border rounded-lg">
                    <Package size={32} className="mx-auto text-light-gray mb-3 sm:w-10 sm:h-10" />
                    <p className="font-display text-base sm:text-lg mb-2">No orders yet</p>
                    <Link href="/products" className="btn-primary mt-4 inline-flex">Start Shopping</Link>
                  </div>
                ) : (
                  <div className="space-y-3 sm:space-y-4">
                    {sampleOrders.map((order) => (
                      <div key={order.id} className="bg-white border border-border rounded-lg p-4 sm:p-5">
            ...
            {activeTab === 'profile' && (
              <div>
                <h2 className="font-display text-lg sm:text-xl mb-4 sm:mb-6">Profile Details</h2>
            ...
            {activeTab === 'addresses' && (
              <div>
                <div className="flex items-center justify-between mb-6">
                  <h2 className="font-display text-lg sm:text-xl">Saved Addresses</h2>
                ...
                {customer?.addresses.length === 0 ? (
                  ...
                  <div className="grid sm:grid-cols-2 gap-3 sm:gap-4">
                    {customer?.addresses.map((addr) => (
                      <div key={addr.id} className="bg-white border border-border rounded-lg p-4 sm:p-5">
